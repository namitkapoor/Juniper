...
// Fixed values for production (Leva is dev-only and heavy)
const FIXED_GLASSES_CONTROLS = {
...
const FIXED_BUZZBASE_CONTROLS = {
  position: { x: 0, y: -0.1, z: 0.5 },
  scale: { x: 0.06, y: 0.06, z: 0.06 },
  rotation: { x: 0.1, y: -4.4, z: 0.5 },
  color: '#4a9eff'
};

const FIXED_IPHONE_CONTROLS = {
  position: { x: 0, y: 0, z: -0.1 },
  scale: { x: 0.08, y: 0.08, z: 0.08 },
  rotation: { x: 0.4, y: 2.7, z: -0.1 }
};
...
export default function Model() {
    ...
    // Use fixed values - Leva removed for production performance
    // In development, you can temporarily import leva if needed for tweaking
    const glassesControls = FIXED_GLASSES_CONTROLS;
    const buzzBaseControls = FIXED_BUZZBASE_CONTROLS;
    const iPhoneControls = FIXED_IPHONE_CONTROLS;
