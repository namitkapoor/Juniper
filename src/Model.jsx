...
export default function Model() {
    const model = useLoader(GLTFLoader, './buzzBase.glb')
    const modelPosition = useControls('model', {position: [0, -1, 0]})
    ...
    useFrame((state, delta) => {
        if (modelRef.current) {
            const rotationSpeed = 5 * delta // Adjust this to control rotation speed
